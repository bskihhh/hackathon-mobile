...
public class MainActivity extends AppCompatActivity implements CardNfcAsyncTask.CardNfcInterface {
    ...
    public void cardIsReadyToRead() {
        ...
        payment.setOrderId(getIntent().getStringExtra("orderId"));
        payment.setAmount( new BigDecimal(getIntent().getDoubleExtra("totalAmount", 100.00f)) );
        payment.setCard(card);
        goboPayClient.pay(payment)
                .enqueue(new Callback<Void>() {

                    @Override
                    public void onResponse(Call<Void> call, Response<Void> response) {
                        Toast.makeText(MainActivity.this, "Paiiiiiid!!!!!", Toast.LENGTH_SHORT).show();
                    }
